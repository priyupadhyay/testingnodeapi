...
router.get('/', function(req, res, next) {
	connection.query('SELECT G.*, U.name as trainer_name FROM groups as G INNER JOIN users as U ON(G.trainer_id = U.user_id) WHERE G.status=1 ', function (error, results, fields) {
	  	if(error){
