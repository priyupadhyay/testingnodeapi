...
router.get('/my/:uid', function(req, res, next) {
	connection.query('SELECT * FROM questions WHERE id IN (SELECT DISTINCT question_id FROM (SELECT * from searches WHERE user_id = ? ORDER BY time) LIMIT 10)',[req.params.uid],[req.params.uid], function (error, results, fields) {
	  	if(error){
	  		res.setHeader('Content-Type', 'application/json');
	  		res.send(JSON.stringify({"status": 500, "error": error, "response": null})); 
	  		//If there is error, we send the error in the error section with 500 status
